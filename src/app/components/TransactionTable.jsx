    ...
    const handleSearch = (e) => {
        ...
        const filtered = orderData.filter(
            (order) =>
              ...
              order.TransactionID.toString().includes(query)
          );
        ...
        <div className="flex flex-col  gap-3 w-full  bg-white p-3 rounded-[4px]">
            <div className="flex flex-row  justify-between ">
                <div className="flex flex-row gap-2 px-4 justify-center items-center py-3.5 text-sm border-[1px] border-dukan-black85 rounded-[4px]">
                    ...
                    <input type="text" className="focus:outline-none bg-white placeholder:text-sm" placeholder="Order Id or Transaction Id" value={searchQuery}
                        onChange={(e)=>setSearchQuery(e?.target?.value)} />
                </div>
                <div className="flex flex-row gap-4">
                    <div className="flex flex-row gap-2 px-4 justify-center cursor-pointer items-center py-3.5 text-sm border-[1px] border-dukan-black85 rounded-[4px]">
                        <p className="text-[16px] text-dukan-black30">Sort</p>
                        <div><Image src={sortUrl} width="14" height="14" alt="sortIcon" /></div>
                    </div>
                    <div onClick={() => convertToCSV(orderData)} className="flex cursor-pointer flex-row gap-2 px-4 justify-center items-center py-3.5 text-sm border-[1px] border-dukan-black85 rounded-[4px]">
                ...
                <tbody >
                    {
                        filteredData?.map((order, index) => {
                            return <tr key={order?.OrderID} className="border-b-[1px] border-dukan-black85 transition-all ease-in-out">
